...
function run_docker() {
   ...
   run_script BEFORE_DOCKER_SCRIPT
    ...
    docker run \
        ...
        $DOCKER_IMAGE /root/$REPOSITORY_NAME/.moveit2_ci/travis.sh
    result=$?
...
function prepare_ros_workspace() {
   ...
   travis_run rosdep install -y -q -n --from-paths . --ignore-src --rosdistro $ROS_DISTRO --skip-keys "libqt5opengl5-dev libqglviewer-dev-qt5"
